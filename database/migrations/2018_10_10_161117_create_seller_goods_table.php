    ...
    public function up()
    {
        Schema::create(
            ...
            function (Blueprint $table) {
                ...
                $table->string('attribute', 255)->comment('商品店铺私有属性：["属性名:属性值"]');
        ...
        Schema::create('seller_goods_gallerys', function (Blueprint $table){
            $table->increments('id');
            $table->unsignedInteger('goods_id')->comment('外键：商品表');
            $table->unsignedInteger('seller_id')->comment('店铺ID');
            $table->string('path')->comment('地址');
            $table->string('ext')->comment('文件类型');
            $table->tinyInteger('sort')->default(255)->comment('排序');
            $table->timestamps();
