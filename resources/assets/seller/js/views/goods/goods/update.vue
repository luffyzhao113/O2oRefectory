<template>
    <i-drawer title="修改商品" :loading="loading" :width="1200">
        <Form ref="formUpdate" :model="formUpdate" :label-width="80" :rules="ruleCreate">
            <Row>
                <Col :span="19">
                    <Row>
                        <Col :span="18">
                            <FormItem label="商品名称" prop="name">
                                <Input v-model="formUpdate.name" placeholder="商品名称"></Input>
                            </FormItem>
                        </Col>
                        <Col :span="24">
                            <Row>
                                <Col :span="6">
                                    <FormItem label="商品条码" prop="bar_code">
                                        <Input v-model="formUpdate.bar_code" placeholder="商品条码"></Input>
                                    </FormItem>
                                </Col>

                                <Col :span="6">
                                    <FormItem label="商品价格" prop="price">
                                        <Input v-model="formUpdate.price" number placeholder="商品价格"></Input>
                                    </FormItem>
                                </Col>

                                <Col :span="6">
                                    <FormItem label="商品库存" prop="stock">
                                        <Input v-model="formUpdate.stock" number placeholder="商品库存"></Input>
                                    </FormItem>
                                </Col>

                                <Col :span="6">
                                    <FormItem label="商品销量" prop="sale">
                                        <Input v-model="formUpdate.sale" number placeholder="商品销量"></Input>
                                    </FormItem>
                                </Col>

                                <Col :span="8">
                                    <FormItem label="上架状态" prop="is_sale">
                                        <true-or-false v-model="formUpdate.is_sale" false-value="下架" true-value="上架"
                                ...
                                <Col :span="8">
                                    <FormItem label="商品分类" prop="category_id">
                                        <group-cascader :change-on-select="false" label="name"
                                                        v-model="formUpdate.category_id" placeholder="商品分类"
                                ...
                                <Col :span="8">
                                    <FormItem label="商品模型" prop="model_id">
                                        <i-select v-model="formUpdate.model_id" transfer clearable
                ...
                <Col :span="4" :offset="1">
                    <div class="base64-image">
                        <FormItem :label-width="0" prop="image">
                            <base64 v-model="formUpdate.image"></base64>
                        </FormItem>
                    </div>
                </Col>
                ...
                <Col :span="10" :offset="1">
                    <FormItem :label-width="0" prop="images">
                        <images v-model="formUpdate.images"></images>
                    </FormItem>
                </Col>
                    ...
                    <Row class="scrollbar attributes">
                        <Col :span="22" :offset="2">
                            <box title="商品属性" v-if="model.attributes.length">
                                <box-item v-for="(item, index) in model.attributes" :key="index" span="8">
                                    <FormItem :label="item.name" class="form-item" :prop="`attribute.${item.name}`"
                                        ...
                                        <i-select transfer v-model="formUpdate.attribute[item.name]" size="small">
                            ...
                            <box title="商品规格" v-if="model.specs.length">
                                ...
                                <template v-for="(item, index) in formUpdate.specs">
                ...
                <Col span="22" offset="1">
                    <FormItem :label-width="0" prop="content">
                        <ueditor v-model="formUpdate.details.content"></ueditor>
                    </FormItem>
                </Col>
            </Row>
        </Form>
        <div slot="footer">
            <Button @click="updateSubmit('formUpdate', `goods/${data.id}`)">更新</Button>
        </div>
    </i-drawer>
</template>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.loading = true;
            this.$http.get(`goods/${this.data.id}/edit`).then((res) => {
        ...
        methods: {
            modelChange(val) {
                ...
                if (model === undefined) {
                    ...
                    model.attributes.forEach((item) => {
                        this.formUpdate.attribute[item.name] = ''
                    })
                }
            },
            handleAdd() {
            ...
            stockChange() {
                ...
                this.formUpdate.specs.forEach((item) => {
                    this.formUpdate.stock += item.stock || 0
                })
            }
        },
        watch: {
            'formUpdate.model_id'(val, old) {
                if(val !== old){
                    let model = this.models.find((v) => v.id === val);
                    if(model !== undefined){
                        this.model = model
